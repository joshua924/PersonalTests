...
public class IpToCidr {
    ...
    private String longToIp(long x, long step) {
        long[] ips = new long[4];
        for (int i = 0; i < 4; i++) {
            ips[i] = x % 256;
            x /= 256;
        ...
        return String.format("%s.%s.%s.%s/%s", ips[3], ips[2], ips[1], ips[0], mask);
    }

    public static void main(String[] args) {
        IpToCidr ip = new IpToCidr();
        System.out.println(ip.ipToCIDR("255.0.0.7", 10));
    }
}
