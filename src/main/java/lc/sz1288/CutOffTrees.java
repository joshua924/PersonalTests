...
public class CutOffTrees {
  ...
  public int cutOffTree(List<List<Integer>> forest) {
    ...
    trees.sort(Comparator.comparingInt(tree -> forest
        .get(tree[0])
        .get(tree[1])));
    int[] position = {0, 0};
    int steps = 0;
  ...
  private int bfs(List<List<Integer>> forest, int x, int y, int targetX, int targetY) {
    ...
    boolean[][] visited = new boolean[forest.size()][forest
        .get(0)
        .size()];
    int steps = 0;
    ...
    while (!positions.isEmpty()) {
      ...
      for (int[] each : positions) {
        for (int[] dir : DIRS) {
          ...
          if (newX >= 0 && newX < forest.size() && newY >= 0 && newY < forest
              .get(0)
              .size() && forest
              .get(newX)
              .get(newY) > 0 && !visited[newX][newY]) {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(cot.cutOffTree(Arrays.asList(Arrays.asList(4, 2, 3), Arrays.asList(0, 0, 1), Arrays.asList(7, 6, 5))));
